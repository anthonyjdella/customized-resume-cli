const themeServer =
  process.env.THEME_SERVER || 'https://resume.anthonydellavecchia.com/theme/';
const fs = require('fs');
...
const denormalizeTheme = (value) => {
  return value.match(/@anthonyjdella(.*)/)[1];
};
...
module.exports = function resumeBuilder(theme, dir, resumeFilename, cb) {
  fs.readFile(resumeFilename, async (err, resumeJson) => {
    if (err) {
      ...
      resumeJson = require('anthonyjdella-resume-schema').resumeJson;
    } else {
      try {
